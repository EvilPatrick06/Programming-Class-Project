...
1. Ubuntu version upgrades (when available)
2. Latest Ubuntu system packages and security updates
3. VS Code extension synchronization across codespaces
4. Latest code FROM GitHub (main or Testing branch)

Shows VS Code dialogs and notifications instead of terminal prompts

Purpose: 
- Check for and install Ubuntu package updates and version upgrades
- Sync VS Code extensions across codespaces
- Pull/sync FROM GitHub TO your codespace
Note: To push your chang    print("🚀 Codespace Auto-Sync Script")
    print("=" * 60)
...
def check_ubuntu_updates():
    ...
    try:
        print("🔍 Checking for Ubuntu package updates...")
        show_vscode_notification("🔍 Checking for Ubuntu updates...", "info")
        ...
        if update_result.returncode != 0:
            print("⚠️  Warning: Could not update package lists")
            return False, 0
        ...
        print(f"❌ Error checking package updates: {e}")
        return False, 0

def check_ubuntu_version_upgrade():
    """Check if a new Ubuntu version/release is available"""
    try:
        print("🔍 Checking for Ubuntu version upgrades...")
        ...
        if "New release" in output and "available" in output:
            ...
            print("✅ Ubuntu version is current (no new LTS release available)")
            return False, "No new LTS release available"
        
        elif "upgrade to the latest non-LTS" in output:
            print("💡 Non-LTS development versions available (LTS upgrade not recommended)")
            return False, "Only non-LTS development versions available"
        
        else:
            print("✅ Ubuntu version appears to be current")
            return False, "No new release detected"
            
    except Exception as e:
        print(f"❌ Error checking Ubuntu version upgrade: {e}")
        return False, f"Error: {e}"

def install_ubuntu_updates():
    """Install Ubuntu updates with user progress feedback"""
    try:
        print("📦 Installing Ubuntu updates...")
        show_vscode_notification("📦 Installing Ubuntu updates... This may take a few minutes.", "info")
        ...
        def show_progress():
            ...
            while upgrade_process.poll() is None:
                print(f"\r📦 Installing updates{'.' * (dots % 4):<4}", end='', flush=True)
                dots += 1
                time.sleep(1)
        ...
        if upgrade_process.returncode == 0:
            show_vscode_notification("✅ Ubuntu updates installed successfully!", "success")
            print("✅ Ubuntu updates installed successfully!")
            ...
            show_vscode_notification("❌ Failed to install some updates", "warning")
            print("❌ Some updates may have failed to install")
            return False
            
    except Exception as e:
        show_vscode_notification(f"❌ Error installing updates: {e}", "error")
...
def handle_ubuntu_updates():
    ...
    try:
        ...
        print("🔄 Checking Ubuntu system status...")
        ...
        if not check_success:
            show_vscode_notification("⚠️  Could not check for Ubuntu package updates", "warning")
            if upgrade_available:
                print(f"💡 However, Ubuntu version upgrade is available: {upgrade_info}")
            return
        
        # Handle version upgrade if available
        if upgrade_available:
            print(f"📈 Ubuntu version upgrade available: {upgrade_info}")
            ...
            if version_upgrade_choice and "Yes" in version_upgrade_choice:
                print("🚀 Starting Ubuntu version upgrade...")
                show_vscode_notification("🚀 Starting Ubuntu version upgrade... This may take 30+ minutes.", "info")
                ...
                if upgrade_result.returncode == 0:
                    show_vscode_notification("✅ Ubuntu version upgrade completed! Restart may be required.", "success")
                    print("✅ Ubuntu version upgrade completed!")
                    print("💡 You may need to restart your codespace to complete the upgrade.")
                else:
                    show_vscode_notification("⚠️  Ubuntu version upgrade had issues. Check manually.", "warning")
                    print("⚠️  Ubuntu version upgrade completed with issues. Please check manually.")
                
                return  # Skip package updates after version upgrade
            
            elif version_upgrade_choice and "Skip all" in version_upgrade_choice:
                show_vscode_notification("⏭️  All Ubuntu updates skipped", "info")
                print("⏭️  All Ubuntu updates skipped")
                return
        
        # Handle package updates
        if update_count == 0:
            if not upgrade_available:
                show_vscode_notification("✅ Ubuntu system is fully up to date!", "success")
                print("✅ Ubuntu packages are already up to date!")
            else:
                print("✅ Ubuntu packages are up to date (version upgrade available but skipped)")
            return
        
        print(f"📦 Found {update_count} package updates available")
        ...
        if "Show me what packages" in install_updates:
            ...
            if proceed and "Yes" in proceed:
                ...
                show_vscode_notification("⏭️  Ubuntu package updates skipped", "info")
                print("⏭️  Ubuntu package updates skipped")
        
        elif "Yes" in install_updates:
            install_ubuntu_updates()
        else:
            show_vscode_notification("⏭️  Ubuntu package updates skipped", "info")
            print("⏭️  Ubuntu package updates skipped. You can install them later with: sudo apt upgrade")
            
    except Exception as e:
        show_vscode_notification(f"❌ Error handling Ubuntu updates: {e}", "error")
        print(f"❌ Error handling Ubuntu updates: {e}")

def handle_vscode_extensions():
    """Handle VS Code extension synchronization"""
    try:
        print("🧩 Synchronizing VS Code extensions...")
        ...
        if result.returncode == 0:
            print("✅ VS Code extensions synchronized successfully!")
            show_vscode_notification("✅ VS Code extensions synchronized!", "success")
            
            # Show a summary of the output
            output_lines = result.stdout.split('\n')
            for line in output_lines:
                if 'Successfully installed' in line or 'Total extensions' in line or 'extensions tracked' in line:
                    print(f"   {line.strip()}")
        else:
            print("⚠️  VS Code extension sync completed with warnings")
            show_vscode_notification("⚠️  Extension sync had some issues", "warning")
            if result.stderr:
                print(f"   Error details: {result.stderr[:200]}...")
        
    except subprocess.TimeoutExpired:
        print("⏰ VS Code extension sync timed out")
        show_vscode_notification("⏰ Extension sync timed out", "warning")
    except Exception as e:
        print(f"❌ Error syncing VS Code extensions: {e}")
        show_vscode_notification(f"❌ Extension sync error: {e}", "error")

def sync_repository():
    """Main sync function with VS Code dialogs - focuses on syncing FROM GitHub"""
    
    # Show welcome message
    show_vscode_notification("🚀 Codespace Auto-Sync Started! Updating system, extensions, and code...", "info")
