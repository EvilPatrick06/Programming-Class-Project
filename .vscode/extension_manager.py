...
def get_installed_extensions():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"⚠️  Warning: Could not get installed extensions: {result.stderr}")
            return []
    except subprocess.TimeoutExpired:
        print("⚠️  Warning: Timeout getting installed extensions")
        return []
    except Exception as e:
        print(f"⚠️  Warning: Error getting installed extensions: {e}")
        return []
...
'''
        ...
        print(f"✅ Updated extensions file with {len(extensions)} extensions")
...
def update_all_extensions():
    ...
    try:
        print("🔄 Updating all extensions to latest versions...")
        ...
        if result.returncode != 0:
            print("⚠️  Could not get extension versions for update check")
            return False
        ...
        if not extensions_to_update:
            print("✅ No extensions found to update")
            return True
        ...
        for ext_id in extensions_to_update:
            try:
                ...
                if result.returncode == 0:
                    success_count += 1
                    print(f"✅ Updated: {ext_id}")
                else:
                    print(f"⚠️  Update warning for {ext_id}: {result.stderr}")
            except Exception as e:
                print(f"❌ Error updating {ext_id}: {e}")
        
        print(f"✅ Successfully updated {success_count}/{len(extensions_to_update)} extensions")
        return True
...
def sync_extensions():
    ...
    show_notification("🔄 Starting VS Code Extension Sync...", "info")
    
    print("🚀 VS Code Extension Manager")
    print("=" * 50)
    print("📦 This script manages VS Code extensions across codespaces")
    print("🔄 Operations: Install missing • Update existing • Track changes")
    print("=" * 50)
    
    # Step 1: Get currently installed extensions
    print("\n📋 Step 1: Scanning installed extensions...")
    installed_extensions = get_installed_extensions()
    print(f"Found {len(installed_extensions)} installed extensions")
    
    # Step 2: Read required extensions from file
    print("\n📄 Step 2: Reading extensions requirements...")
    ...
    if new_extensions:
        ...
        print("✅ No new extensions to track")
    
    # Step 4: Install missing extensions
    print("\n📦 Step 4: Installing missing extensions...")
    missing_extensions = set(required_extensions) - set(installed_extensions)
    
    if missing_extensions:
        print(f"📦 Found {len(missing_extensions)} missing extensions:")
        ...
        if install_success > 0:
            show_notification(f"✅ Installed {install_success} missing extensions!", "success")
    else:
        print("✅ All required extensions are already installed")
    
    # Step 5: Update all extensions
    print("\n🔄 Step 5: Updating extensions to latest versions...")
    update_success = update_all_extensions()
    
    if update_success:
        show_notification("✅ Extension sync completed successfully!", "success")
    else:
        show_notification("⚠️  Extension sync completed with some warnings", "warning")
    
    # Step 6: Final status report
    print("\n📊 Final Status Report:")
    final_installed = get_installed_extensions()
    print(f"   📦 Total extensions installed: {len(final_installed)}")
    print(f"   📄 Extensions tracked in file: {len(required_extensions + list(new_extensions))}")
    print(f"   ✅ Sync operation: {'Completed' if update_success else 'Completed with warnings'}")
    
    print("\n💡 Tips:")
    print("   • Extensions are automatically tracked in .vscode/extensions.txt")
    print("   • Install new extensions normally - they'll be tracked on next sync")
    print("   • This script runs automatically on codespace startup")
    print("   • Manual sync: python3 .vscode/extension_manager.py")
...
def main():
    ...
    try:
        ...
        show_notification("⚠️  Extension sync interrupted by user", "warning")
        print("\n⚠️  Extension sync interrupted by user")
    except Exception as e:
        show_notification(f"❌ Error during extension sync: {e}", "error")
        print(f"❌ Error during extension sync: {e}")
